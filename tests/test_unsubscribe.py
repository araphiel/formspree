...
def test_unconfirm_process(client, msend):
    ...
    f1 = Form("bob@testwebsite.com", host="testwebsite.com", confirmed=True)
    DB.session.add(f1)

    f2 = Form("bob@testwebsite.com", host="othertestwebsite.com", confirmed=True)
    DB.session.add(f2)

    f3 = Form("bob@testwebsite.com", host="anothertestwebsite.com", confirmed=True)
    DB.session.add(f3)
    DB.session.commit()
...
def test_list_unsubscribe_post(client, msend):
    ...
    assert (
        msend.call_args[1]["headers"]["List-Unsubscribe"][1:-1] == list_unsubscribe_url
    )

    r = client.post(list_unsubscribe_url.replace("https", "http"))
    assert r.status_code == 200
