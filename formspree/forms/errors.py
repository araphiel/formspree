...
from formspree.utils import request_wants_json
...
def malformed_replyto_error(status):
    if request_wants_json():
        return (
            jsonify({"error": "_replyto or email field has not been sent correctly"}),
            500,
        )
    ...
    return (
        render_template(
            ...
            text=u'You entered <span class="code">{address}</span>. That is an invalid email address. Please correct the form and try to submit again <a href="{back}">here</a>.<p style="font-size: small">This could also be a problem with the form. For example, there could be two fields with <span class="code">_replyto</span> or <span class="code">email</span> name attribute. If you suspect the form is broken, please contact the form owner and ask them to investigate</p>'
            "".format(address=status["address"], back=status["referrer"]),
        ),
        400,
    )
...
def generic_send_error(status):
    ...
    return (
        render_template(
            ...
            text="Unable to send email. If you can, please send the link to your form and the error information to  <b>{email}</b>. And send them the following: <p><pre><code>{message}</code></pre></p>".format(
                message=json.dumps(status), email=settings.CONTACT_EMAIL
            ),
        ),
        500,
    )
