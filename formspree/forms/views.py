...
def thanks():
    if request.args.get("next") and not valid_url(request.args.get("next")):
        return (
            render_template(
                "error.html", title="Invalid URL", text="An invalid URL was supplied"
            ),
            400,
        )
...
def spam_marking_page():
    if not request.args.get("id"):
        return render_template("error.html", title="Invalid URL", text="An invalid URL was supplied")
    submission = Submission.query.get(spam_serializer.loads(request.args.get("id")))
    if not submission:
        return render_template("error.html", title="Unable to find submission",
                               text="We couldn't find that submission, it may have been deleted.")
    return render_template("forms/mark-spam.html", id=request.args.get("id"))
        ...
        if verify_captcha(request.form, request.remote_addr):
            ...
            if r.ok and r.json()["message"] == "success":
                ...
                return render_template(
                    ...
                    text=email
                    + " is not a valid address or wasn't blocked on our side.",
                )
...
def unconfirm_form(form_id, digest):
    ...
    if request.method == "GET":
        if success:
            ...
            return (
                render_template(
                    "forms/unconfirm.html", other_forms=other_forms, disabled_form=form
                ),
                200,
            )
        else:
            return (
                render_template(
                    ...
                    text="This unconfirmation link is not valid.",
                ),
                400,
            )
...
def unconfirm_multiple():
    ...
    if not unconfirming_for_email:
        return (
            render_template(
                "error.html",
                title="Forbidden",
                text="You're not allowed to unconfirm these forms.",
            ),
            401,
        )
    ...
    return (
        render_template(
            "info.html",
            title="Success",
            text="The selected forms were unconfirmed successfully.",
        ),
        200,
    )
